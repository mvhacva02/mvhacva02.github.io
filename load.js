const raw_text = `word	Nghĩa tiếng Việt và họ của từ
government 	chính phủ
local	địa phương
system 	hệ thống
case	trường hợp
social   	có tính xã hội 
company  	công ty
important 	quan trọng 
state 	tình trạng, trạng thái, quốc gia, bang, 
information 	tin tức
public 	 chung, công cộng; công khai 
head	cái đầu (người, thú); chỉ huy, lãnh đạo, dẫn đầu
fact 	sự thật, sự việc
business 	việc buôn bán, thương mại, kinh doanh 
area  	khu vực, diện tích
perhaps 	có thể, có lẽ
form 	mẫu (đơn), hình thể, hình dạng, hình thức; làm thành 
general  	chung, chung chung; tổng 
possible  	có thể 
council  	hội đồng
develop  	phát triển  
power 	quyền lực sức mạnh, nội lực
country	quốc gia, đất nước 
service 	dịch vụ, sự phục vụ
politics 	chính trị
able  	có năng lực, có tài 
support 	hỗ trợ, ủng hộ 
half 	một nửa
control 	điều khiển, kiểm soát 
major 	đa số, lớn, chủ yếu 
problem	 vấn đề, điều khó giải quyết
court	tòa án, sân (tennis...), 
policy	chính sách
police 	cảnh sát, công an 
report 	 báo cáo, tường trình
interest 	quan tâm, chú ý
probable 	có thể, có khả năng 
available	sẵn có
labour 	 lao động; công việc
law 	 luật 
research 	nghiên cứu
follow 	đi theo sau, theo 
level 	 trình độ, cấp
education 	sự giáo dục, sự rèn luyện (kỹ năng) 
office 	 cơ quan, văn phòng, bộ 
voice 	tiếng, giọng nói
together 	cùng nhau, cùng với
main 	chính, trọng yếu nhất 
care 	sự chăm sóc, chăm nom; chăm sóc
health 	sức khỏe, thể chất 
period 	kỳ, thời kỳ
minister 	 bộ trưởng 
economic 	(thuộc) Kinh tế  
means	 cách, kế sách
effect 	hiệu quả, kết quả
real 	thực, thực tế, có thật 
position 	vị trí, chỗ
community 	dân chúng, nhân dân
view 	nhìn thấy, quan sát, tầm nhìn
staff 	nhân viên
process 	quy trình, quá trình, sự tiến triển, 
particular 	đặc biệt 
provide	cung cấp, chu cấp
present 	hiện tại, hiện thời, có mặt; bày tỏ, giới thiệu, trình bày, món quà
international	quốc tế
moment 	chốc, lát, khoảnh khắc
special 	đặc biệt
result 	kết quả
difficult 	khó, khó khăn difficulty n. /ˈdɪfɪkəlti/ sự khó khăn
sense 	giác quan, cảm giác 
order 	ra lệnh, gọi (món, đặt hàng)
idea 	 ý tưởng
certain 	chắc chắn 
sort 	sắp xếp, phân loại, thứ, hạng, loại 
evidence 	dấu hiệu, chứng cớ
experience 	 kinh nghiệm; trải nghiệm
believe 	 tin, tin tưởng 
turn 	quay, xoay, vặn; sự quay, vòng quay, lượt
range	dãy, hàng, loại; phạm vi, trình độ
matter 	vấn đề, chủ đề
trade 	buôn bán, trao đổi 
industry 	công nghiệp
century 	thế kỷ
course 	khóa học
human 	(thuộc) con người, loài người
unite 	 liên kết, hợp lại 
remember 	 nhớ, nhớ lại
individual 	cá nhân; riêng biệt;
program 	chương trình;
rate 	tỷ lệ, tốc độ
committee 	ủy ban
indeed 	thật vậy, quả thật
language 	 ngôn ngữ
section 	mục, phần
similar 	giống như, tương tự như 
reason 	lý do, lý lẽ 
common 	thông thường, phổ biến
role	vai (diễn), vai trò
nature 	tự nhiên, thiên nhiên
necessary 	cần thiết, thiết yếu 
value 	giá trị, ước tính, định giá 
simply 	một cách dễ dàng, giản dị
especially 	đặc biệt là, nhất là
department 	cục, sở, ban, khoa, phòng
personal 	cá nhân, tư, riêng tư 
figure 	con số lần, thông số, hình dáng
union	liên hiệp, sự đoàn kết
total	tổng số, toàn bộ số lượng 
member 	thành viên, hội viên 
private 	cá nhân, riêng 
type 	 loại, kiểu, mẫu; phân loại, xếp loại
practice 	thực hành, thực tiễn
seem 	có vẻ như, dường như
former 	trước, cũ, xưa
increase 	 tăng, tăng thêm; sự tăng, sự tăng thêm
decision 	n. /diˈsiʒn/ sự quyết định, sự giải quyết, sự phân xử
return 	 trở lại, trở về; sự trở lại, sự trở về
finance 	 tài chính
president 	hiệu trưởng, chủ tịnh, tổng thống
stage 	sân khấu, tầng, bệ
university 	 trường đại học
issue 	phát hành (báo, tiền), đưa ra… (n) vấn đề, bản phát hành (báo, tiền)
tax	thuế; đánh thuế
quality 	 chất lượng
foreign 	 (thuộc) nước ngoài
approach 	đến gần, lại gần; sự đến gần, sự lại gần
rest 	còn lại, sự nghỉ ngơi, lúc nghỉ; nghỉ, nghỉ ngơi 
ground 	 mặt đất, đất, bãi đất
march 	tháng ba, diễu hành
situation 	trường hợp, hoàn cảnh
account 	tài khoản, kế toán; tính toán
offer 	biếu, tặng; đưa ra đề nghị
recent 	gần đây
force 	lực, sức mạnh; ép buộc, cưỡng ép
final 	cuối cùng, chung kết 
secretary 	thư ký
art 	nghệ thuật, mỹ thuật
various 	khác nhau, thuộc về nhiều loại 
field	cánh đồng, lĩnh vực
project 	dự án
forward 	 ở phía trước, tiến về phía trước
include 	bao gồm, tính cả
award 	phần thưởng; tặng, thưởng
hospital 	bệnh viện
record 	hồ sơ, bản ghi, bản thu; thu, ghi chép 
plan 	kế hoạch; lập kế hoạch
chapter 	chương (sách)
royal 	(thuộc) vua, hoàng gia
knowledge 	kiến thức, sự hiểu biết, tri thức
lead 	lãnh đạo, dẫn dắt
deal_with 	giải quyết
series 	loạt, dãy, chuỗi
current 	hiện nay, hiện thời
manage	quản lý, trông nom 
security 	sự an toàn, sự an ninh
structure 	kết cấu, cấu trúc
capital 	 thủ đô, tiền vốn
material 	 nguyên liệu, vật liệu
success	sự thành công,, sự thành đạt 
model 	 mẫu, kiểu mẫu
chance 	cơ hội, sự may mắn
analysis 	sự phân tích  
population	dân cư, dân số
modern 	hiện đại, tân tiến
theory 	lý thuyết, học thuyết
press 	sự ép, nén, ấn 
legal 	hợp pháp 
cause 	 nguyên nhân, nguyên do; gây ra, gây nên
environment 	 môi trường 
design 	thiết kế; sự thiết kế
middle	giữa, ở giữa
relationship 	mối quan hệ
complete 	 hoàn thành, xong
standard 	tiêu chuẩn
list 	danh sách; ghi vào danh sách
space 	khoảng trống, khoảng cách
miss 	 lỡ, trượt; sự trượt, sự thiếu vắng 
property 	tài sản, của cải; đất đai, nhà cửa, bất động sản
term 	điều khoản, khóa, kỳ học
mark 	dấu, nhãn, nhãn mác; đánh dấu, ghi dấu
normal 	thường, bình thường
serious 	nghiêm túc, nghiêm trọng
worth 	đáng giá, có giá trị
begin 	bắt đầu, khởi đầu 
behave 	behaviour (NAmE behavior)thái độ, cách cư xử, đạo đức
previous 	 ưu tiên , trước (vd. ngày hôm trước), 
firm 	 hãng, công ty; chắc, kiên quyết, vũng vàng, mạnh mẽ 
energy 	 năng lượng, nghị lực, sinh lực
direct 	 trực tiếp, thẳng, thẳng thắn; gửi, viết cho ai, điều khiển 
hall 	đại sảnh, tòa (thị chính), hội trường
scheme 	kế hoạch thực hiện, sự sắp xếp, lược đồ
choice 	sự lựa chọn 
significant	đáng kể, quan trọng 
income	thu nhập, lợi tức
contract 	hợp đồng, sự ký hợp đồng, ký kết
risk 	liều lĩnh, mạo hiểm
suddenly	thình lình, đột ngột
continue 	tiếp tục, làm tiếp 
technology 	kỹ thuật, công nghệ
consider 	cân nhắc, xem xét; để ý, quan tâm, lưu ý đến 
defence	 cái để bảo vệ, sự che chở
produce 	 sản xuất, chế tạo 
chairman	 chủ tịch, chủ tọa
activity 	sự hoạt động, sự tích cực
appropriate 	thích hợp, thích đáng
association	sự kết hợp, sự liên kết
allow 	 cho phép
military 	 (thuộc) quân đội, quân sự
army 	 quân đội
attempt 	 sự cố gắng, sự thử; cố gắng, thử
original 	(thuộc) gốc, nguồn gốc, căn nguyên
chief 	chủ tọa, người đứng đầu, trọng yếu
county 	tỉnh
claim 	 đòi hỏi, yêu sách; sự đòi hỏi, sự yêu sách, sự thỉnh cầu
specific 	chính xác, rõ ràng, đặc trưng, riêng biệt 
basic	cơ bản, cơ sở
potential	tiềm năng; khả năng, tiềm lực 
professional 	(thuộc) nghề, nghề nghiệp; chuyên nghiệp
cover 	bao bọc, che phủ;  bìa sách, vỏ bọc
product 	sản phẩm
obviously 	một cách rõ ràng, có thể thấy được, hiển nhiên
unless 	trừ phi, trừ khi, nếu không
appeal 	kêu gọi, khẩn cầu, hâp dẫn,thích thú
demand 	sự đòi hỏi, sự yêu cầu; đòi hỏi, yêu cầu
season 	 mùa
access 	lối, cửa, đường vào
appear 	xuất hiện, hiện ra
investment 	sự đầu tư
independent 	độc lập 
benefit 	lợi ích, phúc lợi
aware 	biết, nhận thức, nhận thức thấy
exactly 	chính xác, đúng đắn
suppose	cho rằng, tin rằng, nghĩ rằng
employment	sự thuê mướn 
popular	đại chúng, (thuộc) nhân dân; ưa chuộng
science 	khoa học, khoa học tự nhiên 
expect	chờ đợi, mong ngóng; liệu trước 
rise 	tăng; đứng lên, mọc (mặt trời)
event 	sự kiện
influence 	sự ảnh hưởng, sự tác dụng; ảnh hưởng, tác động
immediate 	ngay lập tức, tức thì 
commission 	sự ủy nhiệm, mệnh lệnh, hội đồng, ủy ban, 
advice 	lời khuyên, lời chỉ bảo 
concern 	liên quan, dính líu tới; sự liên quan, sự dính líu tới 
contact 	liên lạc, sự giao thiệp; tiếp xúc
charge 	nhiệm vụ, trách nhiệm, lương
ensure 	 bảo đảm, chắc chắn
opportunity 	cơ hội, thời cơ
college	trường cao đẳng
station 	trạm, điểm, đồn
lay	xếp, đặt, bố trí
goods 	hàng hóa, của cải
carry 	mang, vác
useful	hữu ích, giúp ích
lack	sự thiếu; thiếu
operation 	sự hoạt động, quá trình hoạt động
extent 	quy mô, phạm vi
application 	đơn xin, sự ứng dụng
response 	sự trả lời, câu trả lời
degree 	mức độ, trình độ; bằng cấp; độ
average 	trung bình, số trung bình, mức trung bình
region 	vùng, miền, địa phương
site 	chỗ, vị trí
official 	(thuộc) văn phòng; viên chức, chính thức 
accept 	chấp nhận, chấp thuận
title 	đầu đề, tiêu đề; tước vị, tư cách
election 	sự bầu cử, cuộc tuyển cử  
earth 	đất, trái đất
review 	sự xem lại, sự xét lại; làm lại, xem xét lại
considerable 	lớn lao, to tát, đáng kể 
quiet 	lặng, yên lặng, yên tĩnh 
match 	trận thi đấu, đối thủ, địch thủ; đối chọi, sánh được 
supply 	cung cấp, đáp ứng, tiếp tế
physical 	vật chất; (thuộc) cơ thể, thân thể 
extra 	thêm, phụ, ngoại; thứ thêm, phụ
attack	tấn công, công kích
campaign	chiến dịch, cuộc vận động
heavy 	nặng, nặng nề 
complex 	phức tạp, rắc rối
primary 	nguyên thủy, thời cổ đại, sơ cấp, tiểu học
competition 	sự cạnh tranh, cuộc thi, cuộc thi đấu
responsible 	chịu trách nhiệm về
software 	phần mềm (m.tính)
break 	bẻ gãy, đập vỡ; sự gãy, sự vỡ
piece 	mảnh, mẩu; đồng tiền
medical 	(thuộc) y học 
purpose	 mục đích, ý định
rule	quy tắc, điều lệ, luật lệ; cai trị, chỉ huy, điều khiển
trouble	vấn dề, điều lo lắng, điều muộn phiền
notice 	thông báo, chú ý
task 	nhiệm vụ, nghĩa vụ, công việc
separate 	làm rời, tách ra, riêng rẽ
eventually 	cuối cùng
opposition	sự đối lập , sự phản đối; phe đối lập
pattern 	mẫu, khuôn mẫu
method 	phương pháp, cách thức
source 	nguồn
surface 	mặt, bề mặt
division 	sự chia, sự phân chia, sự phân loại
remain 	còn lại, vẫn còn như cũ 
fear 	sự sợ hãi, e sợ, lo ngại
machine 	máy, máy móc
equipment 	 trang thiết bị
suggest 	đề nghị, đề xuất
disease 	căn bệnh, bệnh tật
peace 	hòa bình, sự hòa thuận
balance 	cái cân; làm cho cân bằng, tương xứng
exchange 	trao đổi; sự trao đổi
happen 	xảy ra, xảy đến
sector	khu vực, lĩnh vực
essential 	cốt yếu; yếu tố cần thiết 
finish 	kết thúc, hoàn thành; sự kết thúc, phần cuối 
function	chức năng; họat động
exercise	bài tập, thể dục;
civil 	(thuộc) công dân
status	tình trạng
related (to) 	có liên quan, có quan hệ với ai, cái gì 
character 	tính cách, đặc tính, nhân vật 
safety 	sự an toàn, sự chắc chăn
aid 	sự giúp đỡ; sự viện trợ
flat 	bằng phẳng, dãy phòng, căn phòng, mặt phẳng
 weight 	trọng lượng
transport 	sự vận chuyển, phương tiện đi lại, chuyên chở, vận tải
context 	văn cảnh, khung cảnh, phạm vi
culture	văn hóa
discussion 	sự thảo luận, sự tranh luận
positive 	tích cực, lạc quan
damage 	mối hại, điều hại; làm hư hại, gây thiệt hại
condition 	điều kiện, tình cảnh, tình thế
senior 	nhiều tuổi hơn, kinh nghiệm hơn; lớn tuổi hơn, sinh viên năm cuối
sign 	dấu hiệu, kí hiệu; đánh dấu, viết ký hiệu
league 	liên minh, liên hoàn
budget 	ngân sách
add 	cộng, thêm vào 
patient 	bệnh nhân; kiên nhẫn, nhẫn nại
library 	thư viện
fit 	hợp, vừa; thích hợp, xứng đáng
progress 	sự tiến tới, sự tiến triển; tiến bộ, tiến triển, phát triển
principle 	nguyên lý, nguyên tắc
agree 	đồng ý, tán thành 
create 	sáng tạo, tạo nên
plant 	thực vật, sự mọc lên; trồng, gieo, nhà máy
survey	sự khảo sát; khảo sát
annual 	hàng năm, từng năm 
reach 	đến, đi đến, tới
version 	 phiên bản, bản dịch sang một ngôn ngữ khác
commercial 	buôn bán, thương mại
stock 	kho sự trữ, hàng dự trữ, vốn
protect 	bảo vệ, che chở
duty 	bồn phận, trách nhiệm
executive 	giám đốc điều hành, thi hành
media 	phương tiện truyền thông đại chúng
avoid 	tránh, tránh xa
pretty	khá, vừa phải; xinh, xinh xắn;, đẹp 
race 	cuộc đua; đua 
relevant 	có liên quan, thích hợp,
district 	huyện, quận
double 	cặp đôi, gấp đôi
explain 	giải nghĩa, giải thích 
plus 	 cộng với (số, người...); dấu cộng; cộng, thêm vào
reference 	sự tham khảo, hỏi ýe kiến
huge 	
collection	bộ sưu tập, sự sưu tập
speech 	bài phát biểu, sự nói, khả năng nói, lời nói, cách nói,
effort 	 sự cố gắng, sự nỗ lực
overall 	toàn bộ, toàn thể; tất cả, bao gồm
holiday 	ngày lễ, ngày nghỉ
career 	nghề nghiệp, sự nghiệp
practical 	 thực hành; thực tế 
failure	sự thất bại, người thất bại
speed 	tốc độ, vận tốc
daily 	hàng ngày
scale 	cái cân, hệ thống có chia độ, tỷ lệ
edge 	 lưỡi, cạnh sắc
regular 	 thường xuyên, đều đặn 
memory 	bộ nhớ, trí nhớ, kỉ niệm
studio 	xưởng phim, trường quay; phòng thu
pass 	qua, vượt qua, ngang qua 
cross 	sự băng qua; băng qua, vượt qua, cây Thánh Giá, nỗi thống khổ; 
opinion 	ý kiến, quan điểm
spend 	tiêu, xài
corner 	góc (tường, nhà, phố...)
image 	ảnh, hình ảnh 
impact 	tác động, ảnh hưởng
receive 	nhận, lĩnh
join 	 gia nhập, tham gia; nối,
quarter 	 1/4, 15 phút
express 	diễn tả, biểu lộ, bày tỏ; adj - nhanh, tốc hành 
grant 	cho, ban, tài trợ, đồng ý
instead 	để thay thế instead_of / thay cho
draw	 vẽ, kéo 
credit 	sự tin, lòng tin, danh tiếng; tiền gửi ngân hàng
advantage 	sự thuận lợi, lợi ích, lợi thế
debate 	cuộc tranh luận, cuộc tranh cãi; tranh luận, bàn cãi
pain 	sự đau đớn, sự đau khổ 
clothes 	 quần áo
active 	tích cực hoạt động, nhanh nhẹn
railway 	(BrE) (NAmE railroad ) đường sắt
forest 	 rừng
stuff 	chất liệu, chất
careful 	cẩn thận, cẩn trọng,
length 	chiều dài, độ dài
criminal 	có tội, phạm tội; kẻ phạm tội, tội phạm
strength 	sức mạnh, sức khỏe
except 	 trừ ra, không kể; trừ phi 
conservative 	bảo thủ, giữ gìn, thận trọng
reduce 	 giảm, giảm bớt 
impossible	 không thể làm được, không thể xảy ra
insurance 	sự bảo hiểm
skin 	da, vỏ
warm 	ấm, ấm áp; làm cho nóng, hâm nóng
address 	địa chỉ, đề địa chỉ
confidence 	lòng tin tưởng, sự tin cậy 
message	 tin nhắn, thư tín, điện thông báo, thông điệp
domestic 	 (thuộc) nội trợ, quốc nội, vật nuôi trong nhà,
ordinary 	 thường, thông thường
farm 	trang trại 
fight 	đấu tranh, chiến đấu; sự đấu tranh, cuộc chiến đấu 
require 	đòi hỏi, yêu cầu, quy định 
entire 	toàn thể, toàn bộ 
solution	sự giải quyết, giải pháp
transfer 	dời, di chuyển; sự di chuyển, sự dời chỗ
doubt 	sự nghi ngờ, sự ngờ vực; nghi ngờ, ngờ vực
shape 	hình, hình dạng, hình thù 
distance 	khoảng cách, tầm xa
extremely 	vô cùng, cực độ
challenge 	sự thử thách, sự thách thức; thách thức, thử thách
technical 	 (thuộc) kỹ thuật, chuyên môn 
fresh 	 tươi, tươi tắn 
scene 	cảnh, phong cảnh
correct 	đúng, chính xác; sửa, sửa chữa 
judge 	 xét xử, phân xử; quan tòa, thẩm phán 
target 	mục tiêu, đích,  bia
article 	bài báo, đề mục
achieve 	đạt được, dành được
prevent 	ngăn cản, ngăn chặn, ngăn ngừa
waste 	 lãng phí, uổng phí
animal 	động vật
due 	đến kỳ hạn (trả nợ); xứng đáng, thích đáng
internal 	ở trong, bên trong, nội địa
excellent 	 xuất sắc, xuất chúng
sight 	cảnh đẹp; sự nhìn
traffic 	sự đi lại, sự giao thông, sự chuyển động
touch 	 sờ, mó, tiếp xúc; sự sờ, sự mó, sự tiếp xúc
relief 	sự giảm nhẹ, sự làm cho khuây khỏa
release 	 làm nhẹ, bớt, thả, phát hành; sự giải thoát, thoát khỏi, sự thả, sự phát hành
introduction 	sự giới thiệu, lời giới thiệu
island 	hòn đảo
strange 	 xa lạ, chưa quen  
dry 	làm khô, sấy khô, cạn
initial 	ban đầu, lúc đầu; chữ đầu (của 1 tên gọi) 
existence	sự tồn tại, sự sống
enjoy 	thưởng thức, thích thú cái gì, được hưởng, có được 
wonder 	ngạc nhiên, lấy làm lạ, kinh ngạc, tự hỏi
master 	chủ, chủ nhân, thầy giáo, thạc sĩ
equal 	ngang, bằng;  
religion 	tôn giáo 
famous 	nổi tiếng
contrast	 sự tương phản; làm tương phản
cabinet 	tủ có nhiều ngăn đựng đồ
bridge 	cái cầu
clean 	sạch, sạch sẽ; 
proper 	đúng, thích đáng, thích hợp 
spirit 	tinh thần, tâm hồn, linh hồn 
detail 	chi tiết 
measure 	đo, đo lường; sự đo lường, đơn vị đo lường 
aim 	 mục tiêu, ý định; nhắm, tập trung, hướng vào, sự nhắm (bắn),
tour 	cuộc đo du lịch, cuộc đi dạo, chuyến du lịch; đi du lịch 
prison 	nhà tù 
accident 	tai nạn, rủi ro
fix 	sửa chữa, sửa sang; đóng, gắn, lắp, cố định 
concept 	khái niệm, ý tưởng
fund 	kho, quỹ; tài trợ, tiền bạc, để tiền vào công quỹ
proportion 	sự cân xứng, sự cân đối, tỷ lệ
organization	tổ chức, cơ quan; sự tổ chức
object	đối tượng, vật thể; phản đối, chống lại
spread 	trải, căng ra, bày ra; truyền bá
ahead 	trước, về phía trước
distribute 	phân bổ, phân phối, sắp xếp, phân loại 
search 	sự tìm kiếm, sự điều tra; tìm kiếm, điều tra
factor 	 nhân tố
feature 	nét đặt biệt, điểm đặc trưng; mô tả nét đặc biệt, đặc trưng của...
rural 	(thuộc) nông thôn, vùng nông thôn
display 	bày tỏ, phô trương, trưng bày; sự bày ra, phô bày, trưng bày
raise 	nâng lên, đưa lên, ngẩng lên
substantial 	thực tế, đáng kể, quan trọng 
tend 	có khuynh hướng 
improve 	cải thiện, cái tiến, mở mang
aircraft 	máy bay, khí cầu
background 	phía sau; nền
strategy 	chiến lược
track	dấu vết, đường mòn, đường đua
forget 	quên
content 	nội dung, sự hài lòng
soft 	mềm, dẻo
communication 	 sự giao tiếp, liên lạc, sự truyền đạt, truyền tin
client 	 khách hàng
suit 	 thích hợp, quen, hợp với, bộ com lê, trang phục;
manner 	cách, lối, kiểu; dáng, vẻ, thái độ
pair	đôi, cặp
selection 	sự lựa chọn, sự chọc lọc
attitude 	thái độ, quan điểm
output 	sản phẩm, sản lượng
afraid 	sợ, sợ hãi, hoảng sợ
heat	hơi nóng, sức nóng 
guide 	điều chỉ dẫn, người hướng dẫn; dẫn đường, chỉ đường
profit 	thuận lợi, lợi ích, lợi nhuận
silence 	sự im lặng, sự yên tĩnh
goal 	mục tiêu, gôn, khung thành, bàn thắng (thể thao)
murder	tội giết người, tội ám sát; giết người, ám sát
sufficient 	đủ, thích đáng (+ for) 
agency 	đại lý,môi giới, trung gian 
focus	tập trung; trung tâm
crisis 	sự khủng hoảng, cơn khủng hoảng
conflict 	xung đột, va chạm; sự xung đột, sự va chạm
reform 	cải cách, cải thiện, cải tạo; sự cải cách, sự cải thiện, cải tạo
expensive 	 đắt
capacity 	năng lực, khả năng tiếp thu, năng suất
respect 	sự kính trọng, sự lễ phép; tôn trọng, kính trọng, khâm phục
absolutely 	 tuyệt đối, hoàn toàn
package 	gói đồ, bưu kiện; đóng gói, đóng kiện 
weather 	thời tiết
procedure 	 thủ tục
mental 	(thuộc) trí tuệ, trí óc; mất trí 
bear 	 mang, cầm, vác, đeo, ôm
frequently 	thường xuyên
dangerous 	 nguy hiểm 
recognition 	 sự công nhận, sự thừa nhận
perfect 	hoàn hảo 
secret 	bí mật; điều bí mật 
empty 	trống, rỗng; đổ, dốc; uống, làm cạn
prove 	chứng tỏ, chứng minh
familiar 	thân thiết, quen thuộc
grand 	rộng lớn, vĩ đại
strike 	đánh, đập, bãi công, đình công; cuộc bãi công, cuộc đình công
beat 	tiếng đập, sự đập; đánh đập, đấm
commitment 	lời cam kết
threat 	sự đe dọa, lời đe dọa 
route 	đường đi, lộ trình, tuyến đường
occur 	xảy ra, xảy đến, xuất hiện
violence 	sự ác liệt, sự dữ dội; bạo lực 
element 	yếu tô, nguyên tố
experiment	cuộc thí nghiệm; thí nghiệm
discipline 	kỷ luật
bright 	sáng, sáng chói 
castle 	thành trì, lâu đài
branch 	ngành; nhành cây, nhánh song, ngả đường
audience 	thính, khan giả, khán giả
urban 	 (thuộc) thành phố, khu vực
conversation 	cuộc đàm thoại, cuộc trò chuyện
congress 	đại hội, hội nghị, Quốc hội
desire 	ước muốn; thèm muốn, ao ước
emphasis 	sự nhấn mạnh, tầm quan trọng
seek 	tìm, tìm kiếm, theo đuổi
maintain 	giữ gìn, duy trì, bảo trì
option 	sự lựa chọn
estate 	tài sản, di sản, bất động sản
exhibition 	cuộc triển lãm, trưng bày
debt 	nợ
captain 	người cầm đầu, người chỉ huy, thủ lĩnh
payment 	sự trả tiền, số tiền trả, tiền bồi thường
moral 	 (thuộc) đạo đức, luân lý, phẩm hạnh; có đạo đức 
serve 	phục vụ, phụng sự
surprised 	 ngạc nhiên (+ at)
brief	ngắn, gọn, vắn tắt 
volume 	thế tích, âm lượng
institute 	viện, học viện 
contribute 	đóng góp, ghóp phần 
document 	tài liệu, tư liệu
apparent 	rõ ràng, rành mạch;
code 	mật mã, luật, điều lệ
occasion 	dịp, cơ hội 
enter 	đi vào, gia nhập
faith 	sự tin tưởng, tin cậy; niềm tin, vật đảm bảo 
establish 	lập, thành lập
pleasure 	niềm vui thích, điều thích thú, điều thú vị; ý muốn, ý thích
lie 	nói dối; lời nói dối, sự dối trá
flow 	sự chảy; chảy
gain 	lợi, lợi ích; giành được, kiếm được, đạt tới
literature 	văn chương, văn học
beneath	ở dưới, dưới thấp
gentleman 	người quý phái, người thượng lưu
entry 	sự đi vào,  quyền đi vào
breath 	hơi thở, hơi 
constant 	kiên trì, bền lòng 
attractive	hút, thu hút, có duyên, lôi cuốn
investigation 	sự điều tra, nghiên cứu
encourage 	động viên, ủng hộ 
tradition 	truyền thống 
catch 	bắt lấy, nắm lấy, tóm lấy, chộp lấy
describe	diễn tả, miêu tả, mô tả 
shut 	đóng, khép, đậy; tính khép kín
vital 	(thuộc) sự sống, cần cho sự sống
conclude 	kết luận, kết thúc, chấm dứt (công việc) 
adult	người lớn, người trưởng thành; trưởng thành
silver	bạc, đồng bạc; làm bằng bạc, trắng như bạc
partner 	đối tác; cộng sự
advance 	sự tiến bộ, tiến lên; đưa lên, đề xuất
drug 	thuốc, dược phẩm; ma túy 
palace 	cung điện, lâu đài
narrow	hẹp, chật hẹp
identify 	nhận biết.
engine 	máy, động cơ
owner 	người chủ, chủ nhân
minor	thứ yếu, không quan trọng 
screen	màn hình
affect 	tác động đến 
limit 	Danh từ: sự giới hạn, ranh giới; Động từ: giới hạn, hạn chế 
capable (of)	có năng lực, khả năng trong (vấn đề, lĩnh vực nào đó)
sharp	 sắc, nhọn, bén 
generate	 sinh ra, đẻ ra, tạo ra. 
typical	 tiêu biểu, điển hình, đặc trưng 
journey 	cuộc hành trình
atmosphere	khí quyển
academic	mang tính thuộc học viện, Đại học, viện hàn lâm
definite 	xác định, định rõ, rõ ràng. 
meanwhile	trong lúc đó, trong lúc ấy
exam	kỳ thi 
enable 	cho phép ai làm gì
shoulder	vai
escape 	trốn thoát, thoát khỏi; sự trốn thoát, lỗi thoát
favourite	được ưa thích
brain 	đầu óc, trí não
criticize	phê bình, phê phán, chỉ trích 
negative	phủ định
iron	danh từ: sắt; dộng từ: bọc sắt
sample	mẫu, hàng mẫu
intend	ý định, có ý định 
interview	Danh từ: cuộc phỏng vấn; Động từ: phỏng vấn
decline	(Danh từ) sự suy tàn; (Động từ) suy sụp, suy tàn
fashion	thời trang
coast	bờ biển
valley	thung lũng
magazine	 tạp chí
terrible 	đáng sợ 
phase	giai đoạn, thời kỳ
mention	đề cập đến
customer 	 khách hàng
unfortunate	không may mắn
noise	ồn ào 
revolution 	cuộc cách mạng
index	chỉ số
rare 	hiếm
objective	Danh từ: mục tiêu, mục đích; Tính từ (thuộc) mục tiêu, khách quan
metal	kim loại
motion	sự chuyển động
factory	nhà máy, xí nghiệp, xưởng
injured	bị chấn thương
severe 	nghiêm trọng, trầm trọng.
hole 	 lỗ, lỗ trống; hang
obtain 	đạt được
represent	đại diện
rapid 	nhanh chóng 
contemporary	đương đại
afford 	có đủ khả năng (làm gì)
permanent	vĩnh viễn
contain  	chứa đựng, bao gồm
fundamental 	cơ bản, cơ sở, chủ yếu
request	Danh từ: lời đề nghị, yêu cầu; Động từ: đề nghị, yêu cầu
pension  	tiền trợ cấp, lương hưu
advertise	quảng cáo
crucial	tính cốt yếu
chemical	thuộc hóa học; chất hóa học, hóa chất 
purchase	sự mua, sự tậu; mua, sắm
equivalent	(adj) tương đương; (n) vật tương đương
appoint	bổ nhiệm.
session	buổi họp, phiên họp, buổi, phiên
struggle 	(v) đấu tranh; (n) cuộc đấu tranh
crowd 	đám đông 
virtually  	hầu như, gần như
nation 	dân tộc, quốc gia
channel	kênh (TV, radio)
temperature 	nhiệt độ
accommodation 	sự thích nghi, điều tiết
massive	to lớn, đồ sộ 
stick	dính vào (gì đó); cái gậy, qua củi, cán 
unique	độc nhất vô nhị
aspect  	vẻ bề ngoài, diện mạo
consumer 	người tiêu dùng
author  	tác giả 
interpret  	giải thích
assist 	 trợ giúp, giúp đỡ
lift	nhấc lên; sự nâng, sự nhấc lên
reply  	sự trả lời, sự hồi âm; trả lời, hồi âm
mix	(v) pha, trộn lẫn;(n) sự pha trộn
expert	chuyên gia
argue	tranh cãi, tranh luận 
alive 	vẫn còn sống
suffer	chịu đựng
vision  	sự nhìn, thị lực
fuel  	chất đốt, nhiên liệu
propose   	đề nghị, đề xuất, đưa ra
impress	gây ấn tượng
sheet  	chăn, khăn trải giường; tấm, phiến, tờ
cancer  	bệnh ung thư
enormous  	to lớn, khổng lồ
involve	bao gồm, bao hàm
diet  	chế độ ăn uống, chế độ ăn kiêng
repeat 	nhắc lại, lặp lại
conduct	chỉ đạo, chỉ huy; sự điều khiển, chỉ huy
command 	ra lệnh, chỉ huy; lệnh, mệnh lệnh.
taste  	(n) vị, vị giác; (v) nếm
indicate	chỉ ra (cho chúng ta biết), cho biết
pollution  	sự ô nhiễm
assume	cho rằng (là đúng), giả sử (là đúng),
regard	quan tâm (đến một ai đó/cái gì đó), đánh giá; cái nhìn, sự quan tâm
extend   	mở rộng, kéo dài (thời hạn...), gia hạn,  giơ, duỗi (tay, chân..)
willing	sẵn lòng
shift 	dời chỗ, chuyển, giao; sự thay đổi, sự luân phiên
artist	nghệ sĩ
plastic  	(n) nhựa; (adj) làm bằng nhựa
fill  	làm đấy, lấp kín
comfortable	thoải mái, tiện nghi
wet 	ướt, ẩm ướt
emerge	nổi lên, hiện ra.
promise	(v) hứa; (n) lời hứa
split	(v) tách, chia ra; (n) sự chẻ, sự tách, sự chia ra
remove	dời đi
candidate 	thí sinh, người ứng cử.
victim  	nạn nhân
approval   	sự tán thành, đồng ý, sự chấp thuận
determine	xác định, định rõ
mistake   	sai lầm, lỗi lầm; phạm lỗi, phạm sai lầm
drama   	kịch, tuồng
premises 	biệt thự
theme	đề tài, chủ đề
error 	lỗi, sai lầm
loan  	sự vay mượn
temporary	tạm thời, nhất thời
federal	liên bang
wealth 	sự giàu sang
reflect 	 phản chiếu, phản ánh
handle  	(v) cầm nắm; (n) tay cầm, móc quai
height  	chiều cao, độ cao
treat	đối xử, đối đãi
keen   	ham mê, say mê, ham thích
item 	một thứ gì đó mà đề cập chung chung
bush  	bụi cây, bụi rậm
admit  	thừa nhận
foundation	sự thành lập, tổ chức.
representative	(adj) tượng trưng; (n) đại diện 
chain	dây, xích;
reputation	sự nổi tiếng, danh tiếng.
decade	thập kỷ
resist	chống lại, kháng cự lại
charity	 lòng từ thiện, sự làm từ thiện
prefer 	thích (cái gì đó) hơn
examine	kiểm tra, khám xét
false	sai, nhầm, giả dối
gradually	dần dần, từ từ 
empire 	đế chế, đế quốc
licence	bằng, chứng chỉ,
defeat	(v) đánh thắng, đánh bại; (n) sự thất bại 
deliver	phân phối, phân phát (thư)
global	toàn cầu
emotion	xự xúc động, sự cảm động
protest 	(n) sự phản đối; (v) phản đối
attend	tham dự,
adequate   	đầy, đầy đủ
circle 	đường tròn, hình tròn
furniture  	đồ đạc (trong nhà)
remark	(n) sự nhận xét, phê bình; (v) nhận xét, phê bình.
intelligent	thông minh
rough	gồ ghề, lởm chởm 
survive	sống sót
wave 	(n) sóng, gợn nước; (v) vẫy tay
precise	rõ ràng, chính xác;
fault 	sự thiết sót, sai sót
connect  	kết nối, nối 
brilliant  	tỏa sáng, rực rỡ, chói lọi
depend (on)	phụ thuộc vào, tùy thuộc vào
convention	hội nghị
philosophy 	triết học
pack  	(v) gói, bọc;(n) (một/nhiều) bó, gói
medium	trung bình,
consequence  	kết quả, hậu quả
electronic	(thuộc) điện tử
tie 	(v) buộc, cột, trói;(n) dây buộc, dây trói,
stable	ổn định,
replace	 thay thế
retire	về hưu
enemy  	kẻ thù, quân địch
coach 	 huấn luyện viên
prior	ưu tiên
pure	nguyên chất, tinh khiết,
frame 	khung ảnh
disappear	biến mất,
category	hạng, loại
switch	(n) công tắc; (v) tắt/bật (công tắc)
presumably  	có lẽ (cho rằng là)
landscape 	phong cảnh
affair 	việc
mood 	tâm tính, tính khí, tâm trạng
tension  	sự căng, độ căng, tình trạng căng
arrange	sắp xếp, sắp đặt,
discover	khám phá, phát hiện ra,
roll 	(n) cuốn, cuộn; (v) lăn, cuốn, quấn
tough 	chắc, bền, dai
wage 	tiền lương, tiền công
permit	permission n. /pə'miʃn/ sự cho phép
delay	(n) sự trì hoãn; (v) làm trì hoãn
altogether  	một cách hoàn toàn
routine	thói thường, lệ thường; (adj) thuộc thường lệ, thông thường
presentation	sự thuyết trình
prospect	triển vọng, viễn cảnh
pace	bước chân,
magic 	(n) ảo thuật, ma thuật; ; (adj) (thuộc) ảo thuật, ma thuật, 
prize	giải thưởng
feed  	cho ăn, nuôi
promote 	 thăng chức, thăng cấp
recall 	gợi lại
signal 	(n) dấu hiệu, tín hiệu;(v) ra hiệu, báo hiệu
smooth	nhẵn, trơn
proud 	 tự hào, kiêu hãnh 
perform	biểu diễn
launch  	(v) khai trương; (n) sự hạ thủy, buổi giới thiệu sản phầm
print 	in
occupy	chiếm giữ, chiếm lĩnh
blame 	(v) khiển trách; (n) sự khiển trách
host 	(n) chủ nhà; (v)dẫn (c.trình), đăng cai tổ chức (hội nghị....)
ban 	cấm; sự cấm.
interior  	phía trong; thuộc bên trong, ở phía trong.
register	đăng ký; máy ghi
regret  	hối tiếc; sự hối tiếc
blind 	mù
restrict  	hạn chế
guess	 đoán; sự đoán
entrance  	cổng vào, lối vào
load  	gánh nặng; chất đống
anxious   	lo âu, lo lắng, băn khoăn
shadow 	cái bóng, bóng tối, bóng râm
loose  	lỏng lẻo
nervous 	lo lắng, hoảng sợ.
excuse	(n) lời xin lỗi;(v) xin lỗi.
concentrate	tập trung (vào việc gì đó)
profession  	 nghề, nghề nghiệp
festival  	lễ hội, đại hội liên hoan
conscious	tỉnh táo, có ý thức,
guard  	(n) cái chắn, người bảo vệ;(v) bảo vệ, gác, canh giữ
surround   	vây quanh, bao quanh
depth	chiều sâu, độ dày
awful	tồi tệ, khủng khiếp
guarantee  	(n) sự bảo hành;(n) cam đoan, bảo đảm
musician  	nhạc sĩ 
abandon  	bỏ, từ bỏ
stomach 	dạ dày
soul  	linh hồn, tâm hồn.
slight 	gầy, mỏng manh, thon
prepare  	 sửa soạn, chuẩn bị
draft 	(n) bản phác thảo;(v) phác thảo
outstanding	nổi bật, đáng chú ý
vary  	biến đổi
salt   	muối
harmful  	gây tai hại, có hại
suspect   	(v) nghi ngờ, hoài nghi;(n) người khả nghi, người bị tình nghi
spare  	thừa thãi; đồ dự trữ, đồ dự phòng
junior 	trẻ hơn, ít tuổi hơn; người ít tuổi hơn
honest  	lương thiện, trung thực, chân thật
princess  	công chúa
enthusiastic  	nhiệt tình, hăng hái, say mê.
extraordinary  	lạ thường, khác thường
engage  	đính ước, hứa hẹn, cam kết.
percent	phần trăm
accurate	đúng đắn, chính xác, xác đáng
pride	sự kiêu hãnh, sự hãnh diện.
airport 	sân bay, phi trường
fee	tiền thù lao, học phí
arrive  	đến, tới nơi 
peak	đỉnh, chóp
chamber 	buồng, phòng, buồng ngủ
steady	vững chắc, vững vàng,
observe   	quan sát, theo dõi
evil	xấu, ác; điều xấu, điều ác,
device 	thiết bị, dụng cụ, máy móc
confuse  	nhầm lẫn, gây nhầm lẫn
remote  	xa, xa xôi, xa cách
certificate 	giấy chứng nhận, bằng, chứng chỉ
satisfy	làm thỏa mãn, hài lòng
steam 	 hơi nước
possess 	chiếm hữu 
summary  	bản tóm tắt
upstairs 	ở trên tầng; tầng trên, gác
column	cột, mục (trong văn bản)
pitch 	sân (chơi các môn thể thao)
delight   	sự vui thích, sự vui sướng
approximate	xấp xỉ, gần đúng
disaster 	tai họa, thảm họa
silly  	ngớ ngẩn, ngu ngốc, khờ dại
climate 	khí hậu, thời tiết
dust 	bụi
dirty  	bẩn thỉu, dơ bẩn
deliberate	thận trọng
automatic   	tự động
qualify	đủ khả năng, đủ tư cách
honour	danh dự, thanh danh
grateful	biết ơn
rely on	tin vào, tin cậy, tin tưởng vào
pleasant   	dễ chịu
concrete  	bằng bê tông; bê tông
laboratory	 phòng thí nghiệm
proof 	proof n. /pru:f/ chứng, chứng cớ, bằng chứng; sự kiểm chứng
overcome  	vượt qua (khó khăn)
export 	(v) xuất khẩu; (n) hàng xuất khẩu, sự xuất khẩu
reserve   	đặt trước, dự trữ, để dành.
reveal	tiết lộ, biểu lộ. 
estimate  	(n) sự ước lượng, đánh giá; (v) ước lượng, đánh giá
appreciate 	thấy rõ; nhận thức
universe 	vũ trụ
excite   	kích thích, kích động
oppose	chống đối, đối chọi, đối lập
instrument  	dụng cụ, nhạc cụ
trend  	xu hướng, phương hướng, chiều hướng
resource	tài nguyên
confirm  	xác nhận, chứng thực
desperate	tuyệt vọng, liều mạng
burst	nổ, nổ tung (bom, đạn)
curve 	đường cong
rival  	đối thủ; đối địch
uniform 	đồng phục
stream  	dòng suối
regulation	điều lệ, quy tắc
collapse	đổ, sụp đổ; sự đổ nát, sự sụp đổ
platform	thềm, sân ga
leather 	da (da động vật)
settle	giải quyết, dàn xếp, hòa giải 
retain	giữ lại,
layer  	lớp
stretch	căng ra, duỗi ra, kéo dài ra
relax	giải trí, nghỉ ngơi 
edition  	nhà xuất bản, sự xuất bản
devote	hiến dâng
witness 	sự làm chứng; chứng kiến, làm chứng
bitter	đắng; đắng cay, chua xót
crash 	sự rơi (máy bay); phá tan tành, phá vụn
ultimate	cuối cùng, sau cùng
liquid  	(n) chất lỏng; (adj) lỏng
pipe 	ống dẫn (khí, nước...)
schedule  	(n) kế hoạch làm việc; (v) lên thời khóa biểu
count 	đếm
raw  	sống (chưa chín), còn nguyên chất
ignore	phớt lờ
tongue	lưỡi
asleep  	ngủ, đang ngủ
fall asleep	ngủ thiếp đi
illegal 	trái luật, bất hợp pháp
refuse  	từ chối, khước từ
define 	định nghĩa
reception	sự tiếp nhận
compare  	so sánh, đối chiếu
bother	làm phiền
heaven 	thiên đường
persuade 	thuyết phục
informal  	không nghi thức, không trang trọng
demonstrate	chứng minh, giải thích
guest	khách, khách mời
venture	liều, mạo hiểm,
destroy	phá huỷ, tiêu diệt, 
engineer	 kỹ sư
reverse  	(v) đảo ngược lại; (n) điều trái ngược
alarm 	báo động, báo nguy
generous	rộng lượng
rescue 	giải thoát, cứu nguy; sự giải thoát, sự cứu nguy.
instruction  	sự dạy, tài liệu cung cấp
departure 	sự khởi hành.
repair 	sửa chữa; sự sửa chữa.
grade  	điểm, điểm số; phân loại, xếp loại
drunk  	say rượu
spell  	đánh vần
fancy 	thích, mến
cigarette  	điếu thuốc lá
valid 	có hiệu lực, hợp lý, hiệu quả
rank  	hàng, dãy; sắp xếp thành hàng, dãy
rubbish	rác rưởi, bỏ đi.
habit 	thói quen, tập quán
arrest  	bắt giữ, sự bắt giữ
wire	dây (kim loại)
deny 	từ chối, phản đối, phủ nhận
punish 	phạt, trừng phạt 
mayor  	thị trưởng
explore 	thăm dò, thám hiểm
customs  	hải quan, thuế nhập khẩu
superior  	chất lượng cao
climb   	leo, trèo
humorous 	hài hước, hóm hỉnh
facility 	điều kiện thuận lợi, cơ sở vật chất.
trace	phát hiện, tìm thấy
disappoint	làm thất bại 
pregnant	mang thai
monitor	(n) lớp trưởng, màn hình máy tính;(v) giám sát
root  	gốc, rễ
curious	tò mò
tool 	dụng cụ, đồ dùng
substance	chất, chất liệu
desert 	(n) sa mạc; (v) rời bỏ
resort	khu nghỉ dưỡng
dare	dám (làm gì đó)
sympathy	 sự đồng cảm, 
proceed 	tiếp tục làm một điều gì đó
resident 	cư dân, người trú ngụ
plot  	tình tiết câu chuyện
courage	sự can đảm, sự dũng cảm
thorough   	cẩn thận, kỹ lưỡng
craft  	nghề thủ công
divorce  	sự ly dị
strict 	nghiêm khắc, khắt khe
anniversary	anniversary  
justify   	bào chữa, biện hộ
polish	(n) nước bóng; (v) đánh bóng
discount	sự giảm giá, sự chiết khấu
label  	(n) nhãn, mác;(v) dán nhãn, ghi mác
actor,actress 	diễn viên
convenient	tiện lợi, thuận lợi, thích hợp
recover  	lấy lại, giành lại
deposit 	(n) tiền đặt cọc; (v) đặt cọc
diary 	sổ nhật ký
entertain	giải trí
artificial   	nhân tạo
acquire	dành được, đạt được
salary 	tiền lương
distinguish	phân biệt
weapon	vũ khí
reward	(n) phần thưởng;(v) thưởng, thưởng công.
brush  	(n) bàn chải; (v) chải, quét
frozen	lạnh giá
clerk	thư ký
solve  	giải quyết
aggressive 	hung hăng
breed 	(v) nuôi dưỡng; (n) nòi giống
sail	(v) nhổ neo; (n) cánh buồm
alter	thay đổi, biến đổi,
ambition 	hoài bão, khát vọng
amaze  	làm ngạc nhiên, làm sửng sốt
invite  	mời
impose 	đánh (thuế...)
gear	bánh răng, dụng cụ, đồ dùng.
lecture	bài diễn thuyết, bài thuyết trình
ridiculous  	lố bịch, buồn cười, lố lăng
outline  	(v) vẽ, phác tảo;(n) nét ngoài, hình dáng
concert  	buổi hòa nhạc
creature	sinh vật, loài vật
substitute 	người thay thế, vật thể thới.
dictionary  	từ điển
blank	để trắng, trống
brave  	gan dạ, can đảm
complain 	phàn nàn, kêu ca
instance  	thí dụ, ví dụ; trường hợp cá biệt
soldier  	lính, quân nhân
recommend	đề nghị, giới thiệu, tiến cử
symbol	biểu tượng, ký hiệu
crack	làm nứt, làm vỡ
tropical  	nhiệt đới
manufacture  	(v) chế tạo;(n) sự chế tạo.
muscle 	cơ, bắp thịt
neighbour	hàng xóm
resolve	giải quyết 
expand 	mở rộng
ceremony 	nghi thức, nghi lễ.
enquiry 	sự điều tra, sự thẩm vấn
delicate	thanh nhã, thanh tú, tế nhị
remind	nhắc nhở, gợi nhớ
combine  	kết hợp, phối hợp
explode 	làm nổ, nổ.
burn 	đốt cháy
neat	sạch, ngăn nắp
fence 	hàng rào
encounter 	chạm chán, bắt gặp; sự chạm trán, sự bắt gặp
contest 	cuộc thi, trận đấu
preserve  	bảo quản, giữ gìn
chart 	đồ thị, biểu đồ.
trap	(n) bẫy, cạm bãy;(v) bẫy
ambulance  	xe cứu thương, xe cấp cứu
button 	cái nút, cái khuy,cúc
colleague	bạn đồng nghiệp
disturb	quấy rầy, làm mất yên tĩnh,
restore	phục hồi lại, hoàn lại
crop  	vụ mùa
peaceful  	 yên tĩnh, hòa bình, thái bình,
praise 	(n) sự ca ngợi, sự tán dương; (v) khen ngợi, tán dương
geography  	địa lý, khoa địa lý
insist (on) 	cứ khăng khăng,  cứ nhất định
slide	trượt
depress	làm phiền muộn, làm chán nản
barrier  	chướng ngại vật
rubber  	cao su
indirect 	gián tiếp
rope	dây cáp, dây thừng,
illustrate 	minh họa, làm rõ ý
awkward	vụng về, lung túng
absent 	vắng mặt, nghỉ
gather	tập hợp, thu thập
brand 	nhãn (hàng hóa)
withdraw 	rút, rút khỏi, rút lui
reject	không chấp nhận, loại bỏ
forecast 	(n) sự dự đoán, dự báo;(v) dự đoán, dự báo
flag  	quốc kỳ
flood	lụt, lũ lụt, ; tràn đầy, tràn ngập
adventure 	sự phiêu lưu, mạo hiểm
inform  	báo cho biết, cung cấp tin tức
cure  	(v) chữa trị, điều trị; (n) cách chữa bệnh, cách điều trị
broadcast  	(v) phát ra (trình chiếu),truyền rộng rãi; (n) phát thanh, quảng bá
flavour 	(n) vị, mùi ;(v) cho gia vị, làm tăng thêm mùi vị
acknowledge  	thừa nhận, công nhận
pray  	cầu nguyện
embarrass 	lúng túng,
chase  	(v) săn bắt;(n) sự săn bắt
imply	ngụ ý
custom	phong tục, tập quán
translate	dịch, biên dịch, phiên dịch 
opponent	địch thủ, đối thủ
accent	giọng điệu vùng miền
comedy  	hài kịch
awake 	đánh thức, làm thức dậy
construct  	xây dựng
celebrate 	kỷ niệm, tán dương, ca tụng
capture  	(v) bắt giữ, bắt;(n) sự bắt giữ, sự bị bắt
import  	(n) sự nhập, sự nhập khẩu; (v) nhập, nhập khẩu
forgive 	tha thứ
predict 	 tiên đoán, dự báo
urge	(v) thúc, giục; (n) sự thúc đẩy, sự thôi thúc
powder  	bột, bụi
loyal 	trung thành, trung kiên
exclude  	ngăn chạn, loại trừ
publish	xuất bản
battery  	pin, ắc quy
deserve	đáng, xứng đáng
owe	nợ
convince  	làm cho tin, thuyết phục
convert  	đổi, biến đổi
consist of  	gồm có
consult 	tra cứu, tham khảo, thăm dò, hỏi ý kiến
polite  	lễ phép, lịch sự
receipt 	công thức; đơn thuốc
bargain 	sự mặc cả, sự giao kèo mua bán
fry  	(v) rán, chiên; (n) thịt rán
cheerful	vui mừng, phấn khởi
disadvantage  	sự bất lợi, sự thiệt hại
warn  	báo cho biết, cảnh báo
decrease  	(v)giảm bớt, làm suy giảm;(n) sự giảm đi, sự giảm sút
sincere	chân thành
revise	đọc và sửa lại
announce  	báo, thông báo
adjust	điều chỉnh
signature 	chữ ký
slice 	(n) miếng, lát mỏng;(v) cắt ra thành miếng mỏng, lát mỏng
biology  	sinh vật học
invent	phát minh, sáng chế
sweep 	quét
amuse	làm buồn cười, làm cho vui thích
calculate  	tính toán
dominate	chiếm ưu thế, chi phối
prompt	mau lẹ, nhanh chóng
vegetable 	 rau, thực vật
assure 	đảm bảo, cam đoan
fold	(v) gấp, vén, xắn; (n) nếp gấp
keyboard 	bàn phím
highway	đường quốc lộ
declare  	tuyên bố, công bố
decorate 	rang hoàng, trang trí
lung 	phổi
fridge 	tủ lạnh
exaggerate  	cường điệu, phóng đại 
statue	tượng
float 	nổi, trôi, lơ lửng
freeze	(n) sự đông lạnh, sự giá lạnh;(v) làm đông lạnh. 
plug	phích cắm
steal  	ăn cắp, ăn trộm
accompany	đi theo, đi cùng
sore 	đau, nhức
cancel	hủy bỏ, xóa bỏ
locate  	xác định vị trí, định vị
container	cái đựng, chứa; công te nơ
umbrella 	umbrella n. /ʌm'brelə/ ô, dù
dismiss	giải tán (quân đội, đám đông)
attach	gắn, dán
boil  	sôi, luộc
cough  	(v) ho; (n) sự ho, tiếng ho
admire	khâm phục, thán phục
install 	đặt (hệ thống máy móc, thiết bị...)
insect  	sâu bọ, côn trùng
accidental 	tình cờ, bất ngờ
anticipate	thấy trước, ường trước
insert 	chèn vào, lồng vào
luggage 	hành lý
interrupt  	làm gián đoạn, ngắt lời
congratulations	sự chúc mừng, khen ngợi; lời chúc mừng, khen ngợi
hobby 	sở thích riêng
expose 	 phơi bày, trưng bày,
circumstance 	hoàn cảnh,  tình huống
ingredient 	thành phần
hesitate  	 do dự
dentist 	nha sĩ
pronounce	phát âm 
multiply  	nhân lên
baggage 	hành lý
complicate 	làm phức tạp, rắc rối 
apologize	xin lỗi, tạ lỗi
vacation  	kỳ nghỉ lễ
refrigerator 	tủ lạnh
garbage	rác
annoy	làm phiền, quẫy nhiễu
grocery	cửa hàng tạp phẩm`
const data = raw_text.split('\n').slice(1).reduce((obj, text) => {
    const [en,vi] = text.split('\t').map(t=>t.trim())
    obj[en] = vi;
    return obj
}, {})


// Tạo âm thanh
const sound_true = document.createElement("audio");
sound_true.src = 'audio/true.mp3';
const sound_false = document.createElement("audio");
sound_false.src = 'audio/false.mp3';
function play_sound(sound = true){
    (sound?sound_true:sound_false).play();
}